...
duplicated(agriculture2)
table(duplicated(agriculture2))

duplicated(agriculture_townA)
table(duplicated(agriculture_townA))

duplicated(agriculture_townB)
table(duplicated(agriculture_townB))

#create a unique data set
cleandata <- distinct(agriculture1) # remove duplicates in a dataset
cleandata2 <- unique(agriculture1) # remove duplicates in a dataset

duplicated(cleandata)
table(duplicated(cleandata2))

#appending data in R
agric_AB <- bind_rows(agriculture_townA,agriculture_townB) #bind two or more datasets using rows
#Exercise: check for duplicates in combined data
table(duplicated(agric_AB))

#remove duplicate
agric_AB1 <- distinct(agric_AB)
table(duplicated(agric_AB1))
...
table(agric_gender$location, agric_gender$gender)
chisq.test(table(agric_gender$location, agric_gender$gender))

table(agric_gender$location, agric_gender$Crop_Type)
chisq.test(table(agric_gender$location, agric_gender$Crop_Type))

table(agric_gender$gender, agric_gender$Crop_Type)
chisq.test(table(agric_gender$gender, agric_gender$Crop_Type))
...
shapiro.test(agric_gender$yield_tonnes)
t.test(yield_tonnes ~ gender, data = agric_gender)
...
shapiro.test(agriculture$Fertilizer_Used_tons)
wilcox.test(Fertilizer_Used_tons ~ location, data = agriculture)

shapiro.test(agriculture$`Pesticide_Used(kg)`)
wilcox.test(`Pesticide_Used(kg)`~ location, data = agriculture)
